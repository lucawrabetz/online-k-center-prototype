...
class OfflineMIP:
    ...
    def configure_model(self, instance: FLFullInstance) -> None:
        ...
        '''
        # Add variables:
        self.y = [[self.model.addVar(vtype=GRB.BINARY, name=f"y^{t}_{i}") for i in range(1, t + 1)] for t in range(1, instance.T + 1)]
        self.z = [[[self.model.addVar(vtype=GRB.BINARY, name=f"z^{t}_{i}_{j}") for j in range(0, t + 1)] for i in range(1, t + 1)] for t in range(1, instance.T + 1)]
        self.zeta = [self.model.addVar(vtype=GRB.CONTINUOUS, name=f"zeta^{t}") for t in range(1, instance.T + 1)]
